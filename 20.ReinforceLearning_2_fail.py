...
mainModel.add(keras.layers.Dense(2883, tf.nn.relu, True, "glorot_normal"))
mainModel.add(keras.layers.Dense(961, tf.nn.relu, True, "glorot_normal"))
# mainModel.add(keras.layers.Dense(31, tf.nn.relu, True, 'glorot_normal'))
mainModel.add(keras.layers.Dense(3, kernel_initializer="glorot_normal"))
mainModel.compile(keras.optimizers.Adam(learning_rate=0.01), loss="mse")
...
for i in range(episodeCount):
    ...
    Q_target = (
        rewardsBuffer[startStep:sc]
        + discountRate
        * np.max(targetModel.predict(nextStatesBuffer[startStep:sc]), axis=1)
        * ~terminalsBuffer[startStep:sc]
    )
